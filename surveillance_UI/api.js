...
const Brain = (() => {
  ...
  async function hydrate(camNameById) {
    ...
    employees.forEach(emp => {
      ...
      PEOPLE[key] = {
        ...
        category: 'Employee', name: emp.name, employeeId: emp.external_id || emp.label || null,
        externalId: emp.external_id || null, email: emp.email || null,
  ...
  async function enrollEmployeePhoto({ name, department, email, external_id, images, auth }) {
    const headers = { 'Content-Type': 'application/json' };
    if (auth) headers.Authorization = 'Basic ' + btoa(`${auth.user}:${auth.pass}`);
    const res = await fetch(BASE + '/employees/enroll-photo', {
      method: 'POST', headers,
      body: JSON.stringify({ name, department, email: email || null, external_id: external_id || null, images }),
    });
    if (!res.ok) {
  ...
  // Edit an existing employee's profile fields (name / department / employee-id /
  // email). Only the passed fields change. Admin Basic auth.
  async function updateEmployee(auth, identityId, fields) {
    const headers = { 'Content-Type': 'application/json' };
    if (auth) headers.Authorization = 'Basic ' + btoa(`${auth.user}:${auth.pass}`);
    const res = await fetch(BASE + `/employees/${identityId}`, {
  ...
  // Set a person's FIXED profile photo (base64). Pins it so the pipeline never
  // overwrites the chosen thumbnail with a captured face. Admin Basic auth.
  async function setProfilePhoto(auth, identityId, imageB64) {
    const headers = { 'Content-Type': 'application/json' };
    if (auth) headers.Authorization = 'Basic ' + btoa(`${auth.user}:${auth.pass}`);
    const res = await fetch(BASE + `/identities/${identityId}/photo`, {
      method: 'POST', headers, body: JSON.stringify({ image: imageB64 }),
    });
    if (!res.ok) {
      let detail = `HTTP ${res.status}`;
      try { detail = (await res.json()).detail || detail; } catch (e) {}
