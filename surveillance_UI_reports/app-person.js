/* app-person.js — Reports & Logs site (person-log modal, chart, photo popup).
   Same code as the main console minus the camera-detection sidebar.
   Plain <script> (globals shared across files); loaded in order by index.html. */

/* ---------- Person log modal (Report + Log rows) ---------- */
...
/* Enlarged photo popup.
   ...
   same sighting — one filename stem, one frame, one person. Part 1 writes the
   ...
   people. When no specific sighting is clicked we pick the most recent sighting
   that actually HAS a face and show its whole triple. */
